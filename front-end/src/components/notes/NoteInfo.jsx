...
function NoteInfo(props){
    ...
    return (
        ...
        <div className="note" >
            ...
            <Link to={"/notes/" + note._id + "/edit"} className="btn btn-secondary" title="Edit Note"><EditIcon /></Link>
            <Link to={"/notes/"} className="btn btn-secondary" title="Go back"><CancelIcon /></Link> 
            <button className="btn btn-secondary" onClick={handleDelete} title="Delete Note"> <DeleteIcon /> </button>
            </div>
        </div>
    )
}
